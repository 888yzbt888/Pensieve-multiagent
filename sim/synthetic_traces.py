...
T_l = float(sys.argv[1]) # 每T_l次，变化一次带宽
# random.uniform(1,5)
T_s = float(sys.argv[2]) # 大概是同一带宽最多持续T_s
# random.uniform(0.05, 0.5)，标准差
...
for z in range(1, steps-1):
    transition_probabilities.append(1/switch_parameter*y) #怕是写错了。transition_probabilities.append(1/switch_parameter**z)才对(bug???)(cxr)

# two variance levels
sigma_low = 1.0
sigma_high = 1.0

# probability of switching variance levels，变化variance的概率
variance_switch_prob = 0.2

# probability to stay in same state， 保持state的概率
prob_stay = 1 - 1 / T_l

# takes a state and decides what the next state is
def transition(state, variance): # variance似乎真的没什么用处，还有两个sigma_low，sigma_high
    transition_prob = random.uniform(0,1)

    # pick next variance first
    variance_switch = random.uniform(0,1)
    next_variance = variance
    if ( variance_switch < variance_switch_prob ): # 看这个周期变不变variance
    ...
    if transition_prob < prob_stay: # stay in current state
    ...
    else: # pick appropriate state! 变化state
        next_state = state
        curr_pos = state
        ...
        normalized_transitions = [x/trans_sum for x in curr_transition_probabilities] # 将削断后的数组中的和化为1
        ...
        # now check if there are multiple ways to move this many states away，看是不是上下都可以变化
