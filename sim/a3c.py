...
class ActorNetwork(object):
    ...
    def __init__(self, sess, state_dim, action_dim, learning_rate):
        #传入sess（令整个系统在同一个sess里运行）
        #传入数据维度、学习速率等其他参数
        #创建ActorNetwork
        #从collection中获取scope为'actor'的数据？？作为神经网络参数
        #设定一些placeholder
        #给出梯度计算方法（利用CriticNetwork），用给定梯度的方法更新（Optimizer.apply_gradients）
        self.sess = sess
        ...
        # Set all network parameters，没看懂(cxr)
    ...
    def create_actor_network(self):
        with tf.variable_scope('actor'):
            ...
            split_5 = tflearn.fully_connected(inputs[:, 4:5, -1], 128, activation='relu') #为什么是4：5而不是5：6？
